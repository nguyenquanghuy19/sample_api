...
class SlideDismissal extends StatefulWidget {
  const SlideDismissal({
    Key? key,
    required this.axis,
  ...
  @override
  SlideDismissalState createState() => SlideDismissalState();
}

class SlideDismissalState extends State<SlideDismissal>
  ...
  @override
  void didUpdateWidget(covariant SlideDismissal oldWidget) {
      ...
      assert(() {
        if (resizeAnimation.status != AnimationStatus.forward) {
          ...
          throw FlutterError.fromParts(
            <DiagnosticsNode>[
              ErrorSummary(
                'A dismissed SlideCustomView widget is still part of the tree.',
              ),
              ErrorHint(
                'Make sure to implement the onDismissed handle of the '
                'ActionPane and to immediately remove the SlideCustomView widget from '
