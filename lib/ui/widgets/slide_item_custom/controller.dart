...
/// The direction of a gesture in the context of [SlideCustomView].
...
/// A request made to resize a [SlideCustomView] after a dismiss.
@immutable
class ResizeRequest {
...
/// Represents an intention to dismiss a [SlideCustomView].
@immutable
...
/// Represents the end of a gesture on [SlideCustomView].
@immutable
class EndGesture {
...
/// Represents a gesture used explicitly to open a [SlideCustomView].
class OpeningGesture extends EndGesture {
  /// Creates an [OpeningGesture].
  const OpeningGesture(double velocity) : super(velocity);
}

/// Represents a gesture used explicitly to close a [SlideCustomView].
...
class StillGesture extends EndGesture {
  ...
  /// The direction in which the user dragged the [SlideCustomView].
  final GestureDirection direction;

  /// Whether the user was in the process to open the [SlideCustomView].
  bool get opening => direction == GestureDirection.opening;

  /// Whether the user was in the process to close the [SlideCustomView].
  bool get closing => direction == GestureDirection.closing;
}

/// Represents a way to control a slideCustomView from outside.
class SlideController {
  /// Creates a [SlidableController].
  SlideController(TickerProvider vsync)
  ...
  /// Track the direction in which the slide moves.
  ///
  /// -1 means that the slide is moving to the left.
  ///  0 means that the slide is not moving.
  ///  1 means that the slide is moving to the right.
  ...
  /// The current ratio of the full size of the [SlideCustomView] that is already
  /// dragged.
  ///
  ...
  void _onDirectionChanged() {
    final multiplier = isLeftToRight ? 1 : -1;
    final index = (direction.value * multiplier) + 1;
  ...
  /// Opens the [SlideCustomView.startActionPane].
  ...
  /// Opens the [SlideCustomView.endActionPane].
  ...
  /// Opens the [SlideCustomView] to the given [ratio].
  Future<void> openTo(
    double ratio, {
    Duration duration = _defaultMovementDuration,
    Curve curve = _defaultCurve,
